...
void readGyroscope(double *gyrRaw, int fd){
	...
	gyrRaw[0] = (double)(int16_t)(gyr[1] << 8| gyr[0])*0.07*(3.141592653589793/180);
	gyrRaw[1] = (double)(int16_t)(gyr[3] << 8| gyr[2])*0.07*(3.141592653589793/180);
	gyrRaw[2] = (double)(int16_t)(gyr[5] << 8| gyr[4])*0.07*(3.141592653589793/180);
