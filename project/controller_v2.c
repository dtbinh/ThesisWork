...
static void initMPCXall( struct PosParams *, double *posX_all, double *posU_all, struct AltParams *, double *altX_all, double *altU_all, struct AttParams *, double *attX_all, double *attU_all, double *meas );
	...
	while(1){
		...
		if(sensorReady){		
			...
			if(triggerFly){
				...
				PWM[2] = (  2*mdl_param.b*attU_all[0] + thrust*mdl_param.L*mdl_param.b + mdl_param.L*mdl_param.k*attU_all[2] )/Lbc_mk4; // Motor 4
				PWM[3] = (  2*mdl_param.b*attU_all[1] + thrust*mdl_param.L*mdl_param.b - mdl_param.L*mdl_param.k*attU_all[2] )/Lbc_mk4; // Motor 3
				PWM[1] = ( -2*mdl_param.b*attU_all[0] + thrust*mdl_param.L*mdl_param.b + mdl_param.L*mdl_param.k*attU_all[2] )/Lbc_mk4; // Motor 1
				PWM[0] = ( -2*mdl_param.b*attU_all[1] + thrust*mdl_param.L*mdl_param.b - mdl_param.L*mdl_param.k*attU_all[2] )/Lbc_mk4; // Motor 2
			}
			// If false, force PWM outputs to zero.
				...
				initMPCXall( &posParams, posX_all, posU_all, &altParams, altX_all, altU_all, &attParams, attX_all, attU_all, measBuffer );
