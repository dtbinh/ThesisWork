...
{static uint8_t lastValues_next=0;

    hedge->positionBuffer[lastValues_next].address=
        buffer[16];
    ...
    hedge->positionBuffer[lastValues_next].ready=
        true;
    hedge->positionBuffer[lastValues_next].processed=
        false;
    lastValues_next++;
    ...
    return hedge->positionBuffer[lastValues_next];
}
           ...
           if (addressesNum != 0)
            ...
            if (position.ready)
                printf ("Address: %d, X: %d, Y: %d, Z: %d at time T: %u\n", position.address,
                        position.x, position.y, position.z, position.timestamp);
            hedge->haveNewValues_=false;
        }
    }
}
